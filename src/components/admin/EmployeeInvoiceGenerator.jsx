...
const formatDurationLabel = (hours) => formatDuration(Math.round(hours * 60));
import { COMPANY } from '@/lib/constants';
import jsPDF from 'jspdf';

const CLIENT_INFO = {
  name: `${COMPANY.name} - Naky.fr`,
...
export default function EmployeeInvoiceGenerator({ emp, bookings, clients = [], month, year, existingInvoice, onClose, open }) {
  ...
  const generatePDF = () => {
    ...
    completedBookings.forEach((b, i) => {
      ...
      const hours = durationToHours(b.duration_minutes);
    ...
    doc.setFont('helvetica', 'bold');
    doc.setFontSize(11);
    doc.text(`Total heures : ${formatDurationLabel(totalHours)}`, 17, y);
  ...
  const handleGenerateAndSave = async () => {
    ...
    try {
      ...
      const { path } = await uploadFile(BUCKETS.invoices, file, `employees/${emp.id}`);
      ...
      toast.error('Erreur lors de la génération : ' + (err.message || 'inconnue'));
    } finally {
    ...
    <Dialog open={open} onOpenChange={onClose}>
      <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
        ...
        <div className="bg-[#E95678]/5 border border-[#E95678]/20 rounded-lg p-4 flex justify-between items-center">
          <div>
            <p className="text-sm text-slate-600">Total heures : <strong>{formatDurationLabel(totalHours)}</strong></p>
