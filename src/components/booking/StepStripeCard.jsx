...
export default function StepStripeCard({ data, updateData, errors = {}, onCardReady }) {
  ...
  useEffect(() => {
    const initStripe = async () => {
      ...
      const clientEmail = currentClient?.email || data.contact_details?.email || '';
      const clientName = `${currentClient?.first_name || data.contact_details?.first_name || ''} ${currentClient?.last_name || data.contact_details?.last_name || ''}`.trim();

      const { clientSecret: secret, customerId } = await invokeFunction('create-setup-intent', {
        clientEmail,
        clientName,
      });
