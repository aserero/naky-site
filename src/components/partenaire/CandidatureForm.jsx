...
import { Candidatures } from '@/api/db';
...
export default function CandidatureForm({ onSuccess }) {
  ...
  const handleCVUpload = async (e) => {
    ...
    setUploading(true);
    try {
      const { path } = await uploadFile(BUCKETS.cvs, file);
      set('cv_path', path);
    } catch (err) {
      console.error(err);
      toast.error("L'envoi du CV a échoué. Veuillez réessayer.");
    } finally {
      setUploading(false);
    }
  };
