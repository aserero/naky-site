...
import { Candidatures, Employees } from '@/api/db';
...
export default function AdminCandidatures() {
  ...
  const acceptAndCreateEmployee = async (candidature) => {
    setAccepting(true);
    try {
      // Créer l'employée (avec le coût horaire si renseigné — débloque la facturation)
      await Employees.create({
        ...
        hourly_rate: hourlyRate ? parseFloat(hourlyRate.replace(',', '.')) : null,
      });
      // Mettre à jour la candidature
      await updateMutation.mutateAsync({ id: candidature.id, data: { status: 'accepted' } });
      queryClient.invalidateQueries({ queryKey: ['employees'] });
      toast.success('Candidature acceptée — profil employée créé');
  ...
  const openCv = async (candidature) => {
    try {
      const url = await getSignedUrl(BUCKETS.cvs, candidature.cv_path);
      window.open(url, '_blank', 'noopener,noreferrer');
    } catch (err) {
      toast.error("Impossible d'ouvrir le CV : " + (err.message || 'inconnue'));
    }
  };
                ...
                >
                  <div className="flex justify-between items-start flex-wrap gap-3">
                    ...
                    <div className="flex items-center gap-2 text-xs text-slate-400">
                      ...
                      {c.created_at ? format(parseISO(c.created_at), 'dd MMM yyyy', { locale: fr }) : ''}
                    </div>
                  </div>
          ...
          <DialogContent className="max-w-lg max-h-[85vh] overflow-y-auto">
            ...
            {selected.status === 'pending' && (
              <DialogFooter className="gap-2 mt-4">
                <Button
                  className="bg-green-600 hover:bg-green-700 text-white gap-1.5"
                  onClick={() => { setHourlyRate(''); setShowAcceptDialog(true); }}
                >
      ...
      <Dialog open={showAcceptDialog} onOpenChange={setShowAcceptDialog}>
        <DialogContent>
          ...
          <div className="space-y-1">
            <Label htmlFor="accept_hourly_rate">Coût horaire (€/h) — optionnel</Label>
            <Input
              id="accept_hourly_rate"
              type="number"
              step="0.01"
              min="0"
            ...
            />
            <p className="text-xs text-slate-400">Nécessaire pour générer les factures de l'employée. Modifiable plus tard.</p>
          </div>
          <DialogFooter className="gap-2">
            ...
            >
              <UserCheck className="w-4 h-4 mr-1" /> {accepting ? 'Création...' : 'Confirmer'}
            </Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
