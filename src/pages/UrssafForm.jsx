...
import { UrssafDetails } from '@/api/db';
...
export default function UrssafFormPage() {
  const { currentClient, loading: authLoading, updateClient } = useAuth();
  ...
  useEffect(() => {
    ...
    // Garde d'auth réelle : pas de client connecté → retour à la connexion
    if (!currentClient) {
    ...
    // Pré-remplir : fiche client + détails URSSAF existants (table client_urssaf_details, clé client_id)
    (async () => {
      let details = {};
      try {
        const rows = await UrssafDetails.filter({ client_id: currentClient.id }, 'updated_at');
        details = rows?.[0] || {};
      } catch (err) {
        console.error('Chargement des détails URSSAF impossible :', err);
      }
      if (cancelled) return;
      ...
      setFormData({
        ...
        nom_naissance: details.nom_naissance || currentClient.last_name || '',
        birthdate: details.birthdate || '',
        pays_naissance: details.pays_naissance || 'France',
        zipcode_naissance: details.zipcode_naissance || '',
        email: currentClient.email || '',
        phone: currentClient.phone || '',
        iban: details.iban || '',
        bic: details.bic || '',
        account_holder: details.account_holder || `${currentClient.first_name} ${currentClient.last_name}`,
        numero_voie: details.numero_voie || '',
        lettre_voie: details.lettre_voie || '',
        type_voie: details.type_voie || '',
        nom_voie: details.nom_voie || '',
        lieu_dit: details.lieu_dit || '',
        complement_adresse: details.complement_adresse || '',
        pays: details.pays || 'France',
        zipcode: details.zipcode || currentClient.zipcode || '',
        city: details.city || currentClient.city || ''
      });
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      // La fonction serveur fait tout : upsert des détails URSSAF, urssaf_completed=true,
      // ai_status='completed', webhook Make (Abby) et email de confirmation.
      await invokeFunction('submit-urssaf', {
        clientId: currentClient.id,
        formData,
      });

      // Rafraîchir la fiche client locale (le serveur vient de la mettre à jour)
      await updateClient({ urssaf_completed: true });
